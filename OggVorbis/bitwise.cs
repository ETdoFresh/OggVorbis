...
namespace OggVorbis
{
    public partial class bitwise
    {
        ...
        public static readonly uint[] mask8B =
        {0x00,0x80,0xc0,0xe0,0xf0,0xf8,0xfc,0xfe,0xff};
    }
}

public void oggpack_writeinit(oggpack_buffer* b)
{
    memset(b, 0, sizeof(*b));
...
public void oggpackB_writeinit(oggpack_buffer* b)
{
    oggpack_writeinit(b);
}

public int oggpack_writecheck(oggpack_buffer* b)
{
    if (!b->ptr || !b->storage) return -1;
    return 0;
}

public int oggpackB_writecheck(oggpack_buffer* b)
{
    return oggpack_writecheck(b);
}

public void oggpack_writetrunc(oggpack_buffer* b, long bits)
{
    long bytes = bits >> 3;
    if (b->ptr)
    {
...
public void oggpackB_writetrunc(oggpack_buffer* b, long bits)
{
    long bytes = bits >> 3;
    if (b->ptr)
    {
...
public void oggpack_write(oggpack_buffer* b, unsigned long value, int bits)
{
...
/* Takes only up to 32 bits. */
public void oggpackB_write(oggpack_buffer* b, unsigned long value, int bits)
{
    if (bits < 0 || bits > 32) goto err;
    if (b->endbyte >= b->storage - 4)
    {
...
public void oggpack_writealign(oggpack_buffer* b)
{
    int bits = 8 - b->endbit;
    if (bits < 8)
...
public void oggpackB_writealign(oggpack_buffer* b)
{
    int bits = 8 - b->endbit;
    if (bits < 8)
...
public static void oggpack_writecopy_helper(oggpack_buffer* b,
    void* source, long bits, void (* w)(oggpack_buffer*, unsigned long, int), int msb)
            {
...
public void oggpack_writecopy(oggpack_buffer* b, void* source, long bits)
{
    oggpack_writecopy_helper(b, source, bits, oggpack_write, 0);
}

public void oggpackB_writecopy(oggpack_buffer* b, void* source, long bits)
{
    oggpack_writecopy_helper(b, source, bits, oggpackB_write, 1);
}

public void oggpack_reset(oggpack_buffer* b)
{
    if (!b->ptr) return;
    b->ptr = b->buffer;
    b->buffer[0] = 0;
...
public void oggpackB_reset(oggpack_buffer* b)
{
    oggpack_reset(b);
}

public void oggpack_writeclear(oggpack_buffer* b)
{
    if (b->buffer) _ogg_free(b->buffer);
    memset(b, 0, sizeof(*b));
}

public void oggpackB_writeclear(oggpack_buffer* b)
{
    oggpack_writeclear(b);
}
...
namespace OggVorbis
{
    public partial class bitwise
    {
        ...
        public static void oggpack_readinit(oggpack_buffer b, byte[] buf, int bytes)
        {
            //memset(b, 0, sizeof(*b));
            b.buffer = b.ptr = buf;
            b.storage = bytes;
...
public void oggpackB_readinit(oggpack_buffer* b, unsigned char* buf, int bytes)
{
    oggpack_readinit(b, buf, bytes);
}

/* Read in bits without advancing the bitptr; bits <= 32 */
public long oggpack_look(oggpack_buffer* b, int bits)
{
    unsigned long ret;
    unsigned long m;
...
/* Read in bits without advancing the bitptr; bits <= 32 */
public long oggpackB_look(oggpack_buffer* b, int bits)
{
    unsigned long ret;
    int m = 32 - bits;
...
public long oggpack_look1(oggpack_buffer* b)
{
    if (b->endbyte >= b->storage) return (-1);
    return ((b->ptr[0] >> b->endbit) & 1);
}

public long oggpackB_look1(oggpack_buffer* b)
{
    if (b->endbyte >= b->storage) return (-1);
    return ((b->ptr[0] >> (7 - b->endbit)) & 1);
}

public void oggpack_adv(oggpack_buffer* b, int bits)
{
    bits += b->endbit;
...
public void oggpackB_adv(oggpack_buffer* b, int bits)
{
    oggpack_adv(b, bits);
}

public void oggpack_adv1(oggpack_buffer* b)
{
    if (++(b->endbit) > 7)
    {
        b->endbit = 0;
...
public void oggpackB_adv1(oggpack_buffer* b)
{
    oggpack_adv1(b);
}

namespace OggVorbis { public partial class bitwise {
        /* bits <= 32 */
        public static long oggpack_read(oggpack_buffer b, int bits)
        {
            ...
            ret = b.buffer[b.bufferI + 0] >> b.endbit;
            if (bits > 8)
            {
                ret |= b.buffer[b.bufferI + 1] << (8 - b.endbit);
                if (bits > 16)
                {
                    ret |= b.buffer[b.bufferI + 2] << (16 - b.endbit);
                    if (bits > 24)
                    {
                        ret |= b.buffer[b.bufferI + 3] << (24 - b.endbit);
                        if (bits > 32 && b.endbit != 0)
                        {
                            ret |= b.buffer[b.bufferI + 4] << (32 - b.endbit);
                        }
                    }
                }
            }
        ...
        /* bits <= 32 */
        public long oggpackB_read(oggpack_buffer* b, int bits)
        {
            long ret;
            long m = 32 - bits;
        ...
        public long oggpack_read1(oggpack_buffer* b)
        {
            long ret;
        ...
        public long oggpackB_read1(oggpack_buffer* b)
        {
            long ret;
        ...
        public static long oggpack_bits(oggpack_buffer b)
        {
            return (b.endbyte * 8 + b.endbit);
        }

        public static long oggpackB_bytes(oggpack_buffer b)
        {
            return oggpack_bytes(b);
        }

        public static long oggpackB_bits(oggpack_buffer b)
        {
            return oggpack_bits(b);
        }

        public static byte[] oggpack_get_buffer(oggpack_buffer b)
        {
            return (b.buffer);
        }

        public static byte[] oggpackB_get_buffer(oggpack_buffer b)
        {
...
public static int ilog(unsigned int v)
{
    int ret = 0;
    while (v)
    {
...
public oggpack_buffer o;
public oggpack_buffer r;

public void report(char*in)
{
    fprintf(stderr, "%s", in);
    exit(1);
}

public void cliptest(unsigned long* b, int vals, int bits, int* comp, int compsize)
{
    long bytes, i;
...
public void cliptestB(unsigned long* b, int vals, int bits, int* comp, int compsize)
{
    long bytes, i;
    unsigned char* buffer;
...
public void copytest(int prefill, int copy)
{
...
public void copytestB(int prefill, int copy)
{
    oggpack_buffer source_write;
    oggpack_buffer dest_write;
    oggpack_buffer source_read;
